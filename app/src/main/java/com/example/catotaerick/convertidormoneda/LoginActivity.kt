...
import androidx.appcompat.app.AppCompatActivity
import com.example.catotaerick.convertidormoneda.databinding.ActivityLoginBinding
import com.google.android.material.snackbar.Snackbar // <-- Nueva importación para Material Design
import com.google.android.gms.auth.api.signin.GoogleSignIn
import com.google.android.gms.auth.api.signin.GoogleSignInClient
import com.google.android.gms.auth.api.signin.GoogleSignInOptions
import com.google.android.gms.common.api.ApiException
...
class LoginActivity : AppCompatActivity() {
    ...
    private val googleSignInLauncher = registerForActivityResult(
        ...
        try {
            ...
            mostrarMensaje("Error con Google: ${e.message}")
        }
    }
    ...
    private fun iniciarSesion() {
        ...
        mAuth.signInWithEmailAndPassword(email, password)
            .addOnCompleteListener(this) { task ->
                if (task.isSuccessful) navegarAMain()
                else mostrarMensaje("Correo o contraseña incorrectos")
            }
    }
    ...
    private fun registrar() {
        ...
        if (email.isEmpty() || password.length < 6) {
            mostrarMensaje("El correo debe ser válido y la clave de al menos 6 caracteres")
            return
        }
    ...
    private fun firebaseAuthWithGoogle(idToken: String) {
        ...
        mAuth.signInWithCredential(credential)
            .addOnCompleteListener(this) { task ->
                if (task.isSuccessful) navegarAMain()
                else mostrarMensaje("Error con Google")
            }
    }
    ...
    // Método centralizado para mostrar alertas usando la vista raíz de tu layout
